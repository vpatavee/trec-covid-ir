...
def query(solr, topics_df, metadata_dict, report_name):
    ...
    for i, q in enumerate(topics_df["query"]):
        ...
        q = prepare_query_child_and_title(cleaned_query)
#         print(q)
        solr_query_param = {
            "fl": "id,score",
            "rows": 1000
...
def prepare_query_max(cleaned_query):
    """
    Example:
    {!parent which="abstract:*" score=max}body:covid19+drug
    """
    query = '{!parent which="abstract:*" score=avg}body:'

    return query + "+".join(cleaned_query)

def prepare_query_child_and_title(cleaned_query):
    """
    Example:
    +abstract:covid + {!parent which="abstract:*" score=max}body:covid
    """
    words = " ".join(cleaned_query)
    
#     query = '+abstract:' + "+".join(cleaned_query) + ' +title:' + "+".join(cleaned_query) + ' + {!parent which="abstract:*" score=max}body:' + "+".join(cleaned_query)

    query = '+abstract:({}) +title:({}) +{{!parent which="abstract:*" score=max}}body:{}'.format(words, words, "+".join(cleaned_query))
...
if __name__ == "__main__":
    ...
    python run_3_solr_section_level.py -q ../../CORD-19/2020-06-19/ cord-19-2020-06-19-nested ../tmp/topics-rnd4.xml
    
    """
    
    report_name = "../prepare_query_child_and_title.txt"
